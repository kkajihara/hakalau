...
# I think I need a long form data frame
...
p <- ggplot(rich_df, aes(x=Habitat, y=richness, fill = Host)) +
  geom_boxplot() +
...
# Export
ggsave("../figures/boxplot.png", width = 7, height=5)
...
ro_tukey <- rownames_to_column("Host")
...
ak_tukey <- rownames_to_column("Host")
...
ro_rich_means <- ro_host_richness %>% group_by(Host) %>% summarise(mean = mean(richness))

ak_rich_means <- ak_host_richness %>% group_by(Host) %>% summarise(mean = mean(richness))
