...
ak_shared_rbind <- readRDS("../intermediate_files/rds/ak_shared_rbind.rds")
...
make_relabun_table <- function (rbind_df, habitat) {
  ...
  abun_df <- rbind_df %>%
    ...
    dplyr::summarise(rel_abun_sum=round(sum(rel_abun), digits=3)) %>%
...
write.csv(merged_soil_host_relabun, file="../outputs/soho_relabun_table.csv")
