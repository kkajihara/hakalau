...
append_core_shared <- function (a_host, list_of_dfs) {
  ...
  list_of_dfs[[a_host]] <- mutate(list_of_dfs[[a_host]],
                        Core = ifelse(ASV %in% core_shared_names,
                                        paste("Yes"),
                                        paste("No")))
...
ro_shared_bar <- core_barplot_nosum(ro_shared_rbind,
                                 100,
                                 colors_ten_families,
                                 "\nRemnant Forest")

ak_shared_bar <- core_barplot_nosum(ak_shared_rbind,
                                 100,
                                 colors_ten_families,
                                 "\nRestored Forest")
...
ggsave("../figures/core_shared_asv_barplot.png", width = 11.5, height = 7)

# arguments for noncore are the same as above

ro_shared_bar_noncore <- noncore_nosum(ro_shared_rbind,
                                 100,
                                 colors_ten_families,
                                 "\nRemnant Forest")

ak_shared_bar_noncore <- noncore_nosum(ak_shared_rbind,
                                 100,
                                 colors_ten_families,
                                 "\nRestored Forest")
...
ggsave("../figures/shared_asv_barplot.png", width = 11.5, height = 7)


```


## 2x2 Barplot (generalist and specialist)

```{r}

# unique barplots go in the bottom row, so no titles are needed
...
both_shared_barplots <- ggarrange(ro_shared_bar, ak_shared_bar, labels=c("A)", "B)"), hjust=-0.5, vjust = 5.5, common.legend = TRUE, legend = "right")

both_unique_barplots <- ggarrange(ro_unique_bar_notitle, ak_unique_bar_notitle, labels=c("C)", "D)"), hjust=-0.5, vjust = 1, common.legend = TRUE, legend = "right")



# combine the pre-arranged side by side plots to make a 2x2 plot
all_barplots <- ggarrange(both_shared_barplots, both_unique_barplots, ncol=1, nrow=2, heights = c(1.1, 1)) +
                  theme(plot.margin = margin(0,0,0.5,0.5, "cm"))
all_barplots <- annotate_figure(all_barplots,
...
ggsave("../figures/gen_spec_barplots.png", width=11, height=13)
...
ro_shared_bar_notitle <- core_barplot_nosum_notitle(ro_shared_rbind,
                                                    100,
                                                    colors_ten_families)

ak_shared_bar_notitle <- core_barplot_nosum_notitle(ak_shared_rbind,
                                                    100,
                                                    colors_ten_families)
